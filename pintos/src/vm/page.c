...
page_for_addr(const void *address)
{
	if (address < PHYS_BASE)
	{
		...
		if (address > PHYS_BASE - STACK_MAX) { //Verify page is meant for stack memory
			if (thread_current()->user_esp - 32 < address) { //Verify that the memory size isn't larger than 32 bytes using frame esp member
				return page_allocate(p.addr, false); //Allocate page with given address, and not read only to be eligible for eviction?
			}
		}
	}
	return NULL;
...
page_out(struct page *p)
{
	...
	/* If dirty is true, page has been modified.*/
	dirty = pagedir_is_dirty(p->thread->pagedir, (const void *)p->addr);

	if (p->file == NULL) ok = swap_out(p);

	/* Because page was modified, data must be saved.
	   Write to swap if private, file if not. */
	if (dirty)
	{
		if (p->private)
		{
	...
	/* If page wasn't modified, no problems. */
	else if (!dirty) {
		ok = true;
	}
